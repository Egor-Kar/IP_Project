...
from ip10.settings import SECRET_KEY
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def _authenticate_credentials(self, request, token):
        ...
        try:
            person = MyUser.objects.get(pk=payload['user_id'])
        ...
        if not person.is_verified:
            msg = 'Данный пользователь не подтвердил регистрацию.'
            raise exceptions.AuthenticationFailed(msg)
        # if not person.is_staff:
        #     msg = 'Данному пользователю не подтвердил регистрацию.'
        #     raise exceptions.AuthenticationFailed(msg)
        request.user = person
        return person, token
